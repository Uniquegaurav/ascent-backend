...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	var a domain.Ascent
	source := ""
	if req.FromExploreItemID != "" {
		item, ok := explore.Lookup(req.FromExploreItemID)
		if !ok {
		...
		a = domain.Ascent{Title: strings.TrimSpace(req.Title), Category: req.Category, Theme: orDefault(req.Theme, "ALPINE"), ImageURL: req.ImageURL, Kind: orDefault(req.Kind, "HOBBY"), LocationName: req.LocationName}
	}
	created, err := h.repo.Create(r.Context(), httpx.UserID(r), a, source)
	if err != nil {
